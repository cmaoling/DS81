...
-A INPUT   -s 192.168.0.0/21 -i eth0 -p udp -j ACCEPT
-A INPUT   -i eth0 -j ACCEPT -p udp --dport 53
-A INPUT   -i eth0:1 -j REJECT --reject-with icmp-host-prohibited
-A INPUT   -i eth0 -j REJECT --reject-with icmp-host-prohibited
-A INPUT   -i tun0 -j REJECT --reject-with icmp-host-prohibited
-A INPUT   -j REJECT --reject-with icmp-host-prohibited

-P OUTPUT  DROP
-A OUTPUT -p udp -o tun0 --dport 53 -j ACCEPT
-A OUTPUT -p icmp -o tun0 -j ACCEPT
...
-A FORWARD -o tun0 -i eth0 -s 192.168.0.0/21 -m state --state NEW -j ACCEPT
-A FORWARD -i tun0 -m state --state RELATED,ESTABLISHED -j ACCEPT
-A FORWARD -i tun0 -j DROP
-A FORWARD -o tun0 -j DROP
